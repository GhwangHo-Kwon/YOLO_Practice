...
- 처음에는 **수동으로 라벨링된 데이터**를 사용하여 모델을 학습시켜야 함
    - 이때, 라벨링이 잘 된 데이터셋을 통해 모델이 학습할 수 있도록 함
    - 예를 들어, **이미지 분류**에서는 각 이미지에 대해 **어떤 클래스**(예: 고양이, 개 등)인지를 수동으로 라벨링
    - **객체 탐지**에서는 각 객체의 **경계 상자(bounding box)**와 라벨을 수동으로 생성

**예시**: 초기에는 1000개의 이미지를 직접 라벨링해서 모델을 훈련

### 모델 학습 후 예측 성능 향상

- 초기 모델은 수동으로 라벨링된 데이터를 사용하여 학습
- 초기 모델이 일정 수준 이상으로 학습되면, 예측 성능이 어느 정도 나오기 시작
- 예를 들어, 객체 인식 모델이라면, 학습된 모델이 이미지를 처리하면서 **객체를 예측**
    - 이후 **bounding box**와 **클래스를 예측**할 수 있게 됨
- 초기 모델이 예측하는 라벨은 완벽하지 않음
    - 하지만 학습을 거듭 할수록 **대부분의 경우 유용한 예측**을 할 수 있을 정도로 성능이 개선됨

### 자동 라벨링 추출 (모델을 이용한 자동 라벨링)

- 모델이 일정 수준의 정확도를 갖추면, 수동 라벨링 없이 모델이 예측한 라벨을 **자동 라벨링 데이터**로 활용할 수 있음
- 이 단계에서 모델은 이미지를 **자동으로 라벨링**하여 데이터를 생성
- 예를 들어, 모델이 자동으로 **고양이와 개를 구별**하고, 해당 라벨을 자동으로 할당할 수 있게 됨
    - 모델이 **bounding box**를 생성하고 그 안의 객체가 **고양이**인지 **개**인지를 예측

### 새로운 데이터에 대한 자동 라벨링 및 재학습

- 자동 라벨링을 통해 얻은 새로운 데이터는 **기존 학습 데이터와 합쳐서 다시 학습**할 수 있음
- 이 때 중요한 점은 **자동 라벨링이 100% 정확하지 않음**
    - 따라서, **추가적인 검증 과정**을 거쳐 잘못된 라벨이 포함되지 않도록 해야 함
- **예시**: 자동으로 라벨링된 데이터 중 일부를 **수동으로 검토**하여 라벨의 정확도를 높임
    - 이후 그 데이터를 **재학습에 사용**하여 모델 성능을 지속적으로 개선

### 순차적 학습 (Active Learning)

- 이 과정을 반복하면서 모델의 성능을 **점진적으로 개선**해 나갈 수 있음
- 이러한 접근 방식은 **Active Learning**이라고도 함
- 모델이 **불확실한 예측**을 한 샘플에 대해만 사람의 검토를 요청
    - 이를 통해 학습을 지속적으로 개선하는 방법
- **예시**: 모델이 불확실하게 예측한 샘플에 대해 수동으로 라벨을 수정
    - 이를 학습 데이터로 추가하여 모델 성능을 향상시킴

### 결론

- 자동 라벨링을 위한 **초기 학습 데이터**는 **수동으로 라벨링**해야 함
- 그 이후에는 모델을 통해 **자동으로 라벨링**을 추출하여 데이터를 생성
    - 이를 다시 학습에 활용하는 방식으로 **반복적 학습**을 할 수 있음
- 이렇게 하면 시간이 지남에 따라 **모델의 성능을 지속적으로 향상**시킬 수 있음
- 자동 라벨링과 재학습을 통해 **라벨링 작업의 수고**를 줄이면서도, 모델의 성능을 점차적으로 향상시키는 방식은 매우 효과적인 접근 방법
